...
│                   사용자 인터페이스 계층                       │
│                                                               │
│   inbox/ 파일 드롭  │  CLI 명령  │  한국어 자연어 인터페이스    │
│   (Excel, PDF,     │  (claude)  │  "주보 만들어줘"            │
│    이미지)          │            │  "새신자 등록"              │
└────────┬───────────────────┬────────────────┬────────────────┘
         │                   │                │
┌────────▼───────────────────▼────────────────▼────────────────┐
...
`guard_data_files.py` (PreToolUse hook):

1. Edit/Write 도구 호출 시 대상 파일 경로 추출
2. Write Permission Matrix 대비 호출자 에이전트 확인
3. 인가된 작성자 → exit 0 (허용)
4. 비인가 → exit 2 (차단) + stderr 피드백

### 7.3 YAML 구문 검증

`validate_yaml_syntax.py` (PostToolUse hook):
- `*.yaml` 파일 쓰기 후 `yaml.safe_load()` 실행
- 구문 오류 시 stderr 경고 (exit 0 — 경고 전용)
